...
public class Wegsven {
	...
	public Wegsven(ImageObserver io) {
		myImageObserver = io;
		int zahl = zufallsgenerator.nextInt(3 + 1);
		int modzahl = zahl % 20;
		...
		try {
			...
			if (modzahl == 3) {
				// Bombe
				Wegobject = ImageIO.read(getClass().getClassLoader().getResource(ImagePath +"3"+".png"));// ImagePath + zahl +
	...
	public void paint(Graphics myGraphic, int iXPos, int iYPos) {
		myGraphic.drawImage(Wegobject, iXPos, iYPos, 100, 100, myImageObserver);
	}
}
